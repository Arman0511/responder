...
class AuthenticationService {
  ...
  Future<Either<AppException, None>> forgotPassword(
   ...
   Future<Either<AppException, None>> logoutAdmin() async {
    try {
      await auth.signOut();
      await _sharedPref.deleteCurrentAdmin();
      return const Right(None());
    } catch (e) {
      return Left(AppException(e.toString()));
    }
  ...
  Future<Either<AppException, Admin>> adminlogin(
      {required String email, required String password}) async {
    try {
      final credential = await auth.signInWithEmailAndPassword(
          email: email, password: password);
      ...
      final userId = credential.user!.uid;
      final snap = await db.collection("admin").doc(userId).get();
      final admin = Admin.fromJson(snap.data()!);
      return Right(admin);
    } on FirebaseAuthException catch (e) {
      String errorMessage = e.message.toString();
      if (errorMessage == "wrong-password") {
      ...
      try {
        ...
        return response.fold((l) => Left(AppException(l.message)), (r) async {
          try {
            ...
            await db.collection("responder").doc(r.uid).update(
              {"email": newEmail},
            );
    ...
    if (hasInternet) {
      try {
        ...
        if (currentUser == null) {
          ...
          final updateUserDoc =
              await db.collection('responder').doc(currentUser.uid).get();
  ...
  Future<Either<AppException, None>> adminSignup(
      String name, String email, String password, String phoneNum) async {
    try {
      UserCredential credential = await auth.createUserWithEmailAndPassword(
        email: email,
